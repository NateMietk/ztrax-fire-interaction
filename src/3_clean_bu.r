  ...
  if (!file.exists(file.path(anthro_out, 'building_counts', 'building_counts_all', 'sum_ecoregion_bu.rds'))) {
    ...
    sum_extractions_bu <- bu_stack$extract(sp = ecoregions, fun = function(x){sum(x, na.rm=TRUE)}) %>%
      ...
      mutate(fpa_id = as.data.frame(fpa)$FPA_ID,
             ecoregion_id = data.frame(ecoregions)$us_l3name,
             sum_bu_1990 = V1,
             sum_bu_1995 = V2,
             sum_bu_2000 = V3,
             sum_bu_2005 = V4,
...
sum_extractions_bu <- bu_stack$extract(sp = fpa, df = TRUE, fun = function(x){sum(x, na.rm=TRUE)}) %>%
  as_tibble() %>%
  mutate(fpa_id = as.data.frame(fpa)$FPA_ID,
         sum_bu_1990 = V1,
         sum_bu_1995 = V2,
...
# setup parallel environment
mean_extractions_bu <- bu_stack$extract(sp = fpa, df = TRUE, fun = function(x){mean(x, na.rm=TRUE)}) %>%
  as_tibble() %>%
  mutate(fpa_id = as.data.frame(fpa)$FPA_ID,
         mean_bu_1990 = V1,
         mean_bu_1995 = V2,
...
# setup parallel environment
median_extractions_bu <- bu_stack$extract(sp = fpa, df = TRUE, fun = function(x){median(x, na.rm=TRUE)}) %>%
  as_tibble() %>%
  mutate(fpa_id = as.data.frame(fpa)$FPA_ID,
         median_bu_1990 = V1,
         median_bu_1995 = V2,
